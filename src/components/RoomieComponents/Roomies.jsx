...
export function RoomiesList() {
    ...
    const handleFilter = (filters) => {
        ...
        setFilteredUsers(
          users.filter((user) => {
            const matchComuna = comuna ? user.Usuario_Roomie.Ubicacion === comuna : true;
            const matchIntereses = intereses.length
              ? intereses.every((interes) => user.Usuario_Roomie.Intereses.split(",").includes(interes))
              : true;
            const matchPreferencias = preferencias.length
              ...
              : true;
              const matchCarrera = carrera ? String(user.Id_carrera) === String(carrera) : true;
      
            return matchComuna && matchIntereses && matchPreferencias && matchCarrera;
          })
        );
      };
