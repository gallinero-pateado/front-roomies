...
export function RoomieCard({
  ...
  const [imageSrc, setImageSrc] = useState(Foto_perfil);

  // Función que cambia la URL de la imagen a la de fallback cuando ocurre un error
  const handleError = () => {
    const iniciales = Nombres.split(' ').map((nombre) => nombre.charAt(0)).join('');
    setImageSrc(`https://ui-avatars.com/api/?name=${iniciales}&size=128&background=random`);
  };
  ...
  return (
    <article className={`${styles.card} shadow-md rounded-lg p-4 w-full max-w-[900px] mx-auto`}>
      <header className="flex flex-col sm:flex-row items-center mb-4">
        <img
          className="w-16 h-16 rounded-full mr-4 mb-4 sm:mb-0"
          src={imageSrc}
          alt={`${Nombres} perfil`}
          onError={handleError}  // Ejecuta handleError si falla la carga de la imagen
        />
