...
export function RoomieCard({
  ...
  return (
    <article
      className={`${styles.card}  shadow-md rounded-lg p-4 min-w-[900px]`}
    >
      ...
      <aside className="flex justify-between">
        <button
          className={`py-2 px-4 rounded ${
            isFav ? "bg-[#FFD166] text-[#1D4157]" : "bg-gray-200 text-black"
          }`}
          onClick={handleClick}
        >
        ...
        <div>
          <button
            type="button"
            className="py-2 px-4 bg-[#0092BC] text-white rounded"
            onClick={openModal}
          >
            Ver perfil
          </button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div
            className={`${styles.card}  rounded-lg shadow-lg p-5 w-full max-w-lg min-w-[850px] max-h-[90vh] overflow-y-auto`}
          >
              ...
              <div className="w-1/2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {Intereses.map((interes, index) => (
                    <p
                      key={index}
                      className="text-sm bg-[#0092BC] text-white px-2 py-1 rounded-full"
                    >
                      {interes}
                    </p>
                  ))}
            ...
            <form onSubmit={sendMessage}>
              <div className="mb-4 flex flex-col">
                <label htmlFor="Asunto" className="font-bold">
                  Asunto:
                </label>
                <input
                  type="text"
                  className={`${styles.inputText} ${styles.inputBg} rounded border border-gray-600 my-3 py-1`}
                  value={subject}
